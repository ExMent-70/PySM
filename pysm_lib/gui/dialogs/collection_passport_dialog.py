...
from ...models import ScriptRootModel, ScriptSetsCollectionModel, ScriptSetEntryModel
...
class CollectionPassportDialog(QDialog):
    def __init__(
        ...
        self.setWindowTitle(self.locale_manager.get("dialogs.collection_passport.title"))
        ...
        self.name_label = QLabel(self.locale_manager.get("dialogs.collection_passport.name_label"))
        ...
        self.description_label = QLabel(self.locale_manager.get("dialogs.collection_passport.description_label"))
        ...
        self.properties_toolbox.addItem(props_widget, self.locale_manager.get("dialogs.collection_passport.properties_group"))

        roots_widget = QWidget()
        roots_layout = QVBoxLayout(roots_widget)
        start_dir_for_roots = str(collection_file_path.parent if collection_file_path and collection_file_path.is_file() else APPLICATION_ROOT_DIR / "scripts")
        self.roots_editor = PathListEditor(
            locale_manager=self.locale_manager,
            dialog_title=self.locale_manager.get("dialogs.collection_passport.select_dir_dialog_title"),
            allow_editing=False,
            default_dialog_path=start_dir_for_roots,
        )
        self.roots_editor.set_paths([root.path for root in self.collection_model.script_roots])
        roots_layout.addWidget(self.roots_editor)
        self.properties_toolbox.addItem(roots_widget, self.locale_manager.get("dialogs.collection_passport.roots_group"))
        ...
        context_group = QGroupBox(self.locale_manager.get("dialogs.collection_passport.context_group"))
        ...
        self.context_editor = ContextEditorWidget(
            ...
            get_script_name_func=get_script_name_func,
        )
        ...
        self.description_edit.textChanged.connect(lambda: self.ok_button.setEnabled(True))
        self.context_editor.data_changed.connect(lambda: self.ok_button.setEnabled(True))
