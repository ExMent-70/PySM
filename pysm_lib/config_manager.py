...
class ConfigManager:
    ...
    def _save_to_file(self, model_to_save: AppConfigModel) -> bool:
        try:
            ...
            # --- ИЗМЕНЕНИЕ: Преобразуем путь к интерпретатору в относительный, если возможно ---
            config_copy_for_save.paths.python_interpreter = to_relative_if_possible(
                config_copy_for_save.paths.python_interpreter,
                base_dir=APPLICATION_ROOT_DIR,
            )

            # Путь к последней коллекции всегда сохраняется относительно корня приложения
