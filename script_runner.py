...
# --- ИЗМЕНЕНИЕ: Добавлен импорт sys для использования sys.executable ---
import sys
import subprocess
import threading
import os
...
class ScriptRunner:
    def __init__(
        ...
        # --- ИЗМЕНЕНИЕ: Проверка существования интерпретатора и fallback на sys.executable ---
        if python_interpreter and os.path.exists(python_interpreter):
            self.python_interpreter: str = python_interpreter
        else:
            # Используем текущий интерпретатор Python как гарантированно рабочий вариант
            self.python_interpreter: str = sys.executable
    ...
    def _prepare_environment(self) -> Dict[str, str]:
        ...
        # --- ИЗМЕНЕНИЕ: Используем уже проверенный интерпретатор вместо сложной логики ---
        interpreter_path = self.python_interpreter
        interpreter_path_obj = pathlib.Path(interpreter_path)
    ...
    def _read_stream(self, stream_pipe, stream_name: str, script_id_cb: str):
        ...
        try:
            for line in iter(stream_pipe.readline, ""):
                ...
                # Проверяем "богатый" контент ТОЛЬКО в потоке stderr
                ...
                # Выводим обычный текст, если вывод не заблокирован
    ...
    def run(self, effective_id_for_callbacks: str):
        ...
        # --- ИЗМЕНЕНИЕ: Добавлена диагностическая информация для отладки ---
        logger.debug(f"Using Python interpreter: {self.python_interpreter}")
        logger.debug(f"Interpreter exists: {os.path.exists(self.python_interpreter)}")
        # ------------------------------------------------------------
        ...
        # --- ИЗМЕНЕНИЕ: Логируем команду для отладки проблем выполнения ---
