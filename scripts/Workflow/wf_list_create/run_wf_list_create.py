...
class ClassListEditor(QMainWindow):
    ...
    def _save_for_processing(self) -> None:
        if self.table_model.rowCount() == 0: return
        if self.config.wf_output_txt_file: output_path = pathlib.Path(self.config.wf_output_txt_file)
        elif self.config.wf_dest_dir: output_path = pathlib.Path(self.config.wf_dest_dir) / CHILDREN_LIST_FILENAME
        else: self.statusBar().showMessage("Путь не определен.", 4000); return
