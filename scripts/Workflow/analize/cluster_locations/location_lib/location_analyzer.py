...
class LocationAnalyzer:
    ...
    def _initialize_clip_model(self) -> Tuple[ort.InferenceSession, Dict[str, str], Dict[str, str]]:
        ...
        for key, possible_names in required_input_names.items():
            ...
            if not found_name:
                raise RuntimeError(f"Не найден входной тензор для '{key}'. Доступно: {available_inputs}")
        ...
        if not all(found_output_names.values()):
            raise RuntimeError(f"Не найдены выходные тензоры 'image_embeds'/'text_embeds'. Доступно: {available_outputs}")
    ...
    def get_image_embedding(self, mask_path: Path) -> Optional[Tuple[Path, np.ndarray]]:
        ...
        try:
            ...
            if img is None:
                logger.error(f"Не удалось декодировать маску: {mask_path.name}")
                return None

            input_tensor = self._preprocess_image(img)
            
            # Пустые текстовые входы для Image-only инференса
            ...
            logger.error(f"Ошибка обработки {mask_path.name}: {e}", exc_info=True)
            return None

    def get_text_embeddings(self, prompts: List[str]) -> np.ndarray:
        if not TRANSFORMERS_AVAILABLE:
            raise ImportError("Библиотека 'transformers' не найдена. `pip install transformers`")
        
        if not self.tokenizer:
            self._init_tokenizer()

        print(f"Вычисление эмбеддингов для <b>{len(prompts)}</b> промптов...")
    ...
    def _init_tokenizer(self):
        ...
        try:
            if local_tokenizer_path.exists() and any(local_tokenizer_path.iterdir()):
                logger.info(f"Загрузка токенизатора из локального кеша: {local_tokenizer_path}")
                self.tokenizer = CLIPTokenizer.from_pretrained(str(local_tokenizer_path))
            else:
                logger.info(f"Локальный токенизатор не найден. Скачивание {hf_model_name}...")
                print("PYSM_CONSOLE_BLOCK_START")
