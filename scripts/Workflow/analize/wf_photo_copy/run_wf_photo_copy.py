...
class Worker(QThread):
    ...
    def _task_search(self):
        ...
        for num in self.numbers_to_find:
            ...
            child_name = ""
            try: child_name = item_data.get("faces", [{}])[0].get("child_name", "")
            ...
            results.append({
                ...
                "child_name": child_name, "location_name": item_data.get("location_name", "Не определена"),
...
class SorterWindow(QMainWindow):
    ...
    def __init__(self, config: Namespace):
        ...
        try:
            portrait_path = pathlib.Path(config.portrait_json)
            group_path = pathlib.Path(config.group_json)
            self.data_manager = JsonDataManager(portrait_json_path=portrait_path, group_json_path=group_path)
            if not self.data_manager.load_data(): raise IOError("Ошибка загрузки данных JsonDataManager.")
            self.metadata = {**self.data_manager.group_data, **self.data_manager.portrait_data}
            
            matches_path_str = str(group_path.parent / "matches_portrait_to_group.json")
            self.matches_json_path = pathlib.Path(matches_path_str)
            self.matches_data = _read_json_safely(self.matches_json_path)
        except Exception as e:
