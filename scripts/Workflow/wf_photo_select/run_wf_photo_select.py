...
def get_config() -> Namespace:
    ...
    parser.add_argument("--__fs_source_dir", type=str, help="Папка для поиска файлов.")
...
# 3. БЛОК: Иерархическая модель данных для TreeView
# ==============================================================================
class TreeItem:
...
class Worker(QThread):
    ...
    def _task_parse_text(self):
        ...
        if format_span_box_markup and self.palette:
            try:
                # --- НАЧАЛО ИЗМЕНЕНИЙ ---
                # Заменяем метку "Номер" на пустую строку "", чтобы текст не отображался
    ...
    def _task_search_files(self):
        ...
        all_files_in_dir = []
        select_folder_path = source_path / SELECTED_FOLDER
        for p in source_path.rglob("*"):
            if select_folder_path in p.parents: continue
            if p.is_file(): all_files_in_dir.append(p)
        all_files_in_dir.sort(key=lambda x: x.name)
        logger.info(f"Всего файлов для анализа (исключая {SELECTED_FOLDER}): {len(all_files_in_dir)}")
    ...
    def _task_execute_operation(self):
        source_path = pathlib.Path(self.source_dir)
        dest_root = source_path / SELECTED_FOLDER
...
# 5. БЛОК: Главное окно приложения
# ==============================================================================
class FileSelectorWindow(QMainWindow):
    ...
    def _init_ui(self):
        ...
        source_dir = getattr(self.config, '__fs_source_dir', None)
        if IS_MANAGED_RUN and source_dir:
            win_title += f" - [{pathlib.Path(source_dir).name}]"
    ...
    def _start_parse(self):
        source_dir = getattr(self.config, '__fs_source_dir')
        if not source_dir or not pathlib.Path(source_dir).is_dir():
    ...
    def _start_search(self):
        ...
        self.worker.source_dir = getattr(self.config, '__fs_source_dir')
    ...
    def _start_operation(self):
        ...
        if QMessageBox.question(self, "Подтверждение", f"Вы уверены, что хотите {op_name_verb} {len(files_to_process)} файлов в '{SELECTED_FOLDER}'?", QMessageBox.Yes | QMessageBox.No, QMessageBox.No) == QMessageBox.No: return
    ...
    def get_files_from_item(self, item: TreeItem) -> List[pathlib.Path]:
        files, base_dir = [], pathlib.Path(getattr(self.config, '__fs_source_dir'))
    ...
    def add_final_links(self):
        if IS_MANAGED_RUN and self.final_status == 0 and pysm_context:
            try:
                source_path = pathlib.Path(getattr(self.config, '__fs_source_dir'))
                select_path = source_path / "Select"
                if select_path.exists(): pysm_context.log_link(url_or_path=str(select_path), text="<br>Открыть папку с отобранными файлами ('Select')")
                pysm_context.log_link(url_or_path=str(source_path), text="Открыть исходную папку")
            except Exception as e:
