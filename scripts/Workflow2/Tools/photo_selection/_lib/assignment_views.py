...
class AssignmentViewsMixin:
    ...
    def _render_assignment_views(self) -> None:
        ...
        if result is None:
            return
        self._rebuild_preview_index()
        self._render_photo_table(result)
        self._render_student_location_table(result)
        self._render_assignment_summary()

    def _render_photo_table(self, result: BuildResult) -> None:
        visible = [
        ...
        for record in sorted(visible, key=lambda item: item.number):
            files = self._file_entries(record)
            compact_students = (
    ...
    def _render_student_location_table(self, result: BuildResult) -> None:
        ...
        for student in sorted(self.roster.students, key=lambda item: item.display_name.casefold()):
            records_by_location = {
                location: sorted(
                    (
                        ...
                        and student.student_id in record.assigned_student_ids
                    ),
            ...
            student_item = QTreeWidgetItem(table, (student.display_name, status, "", "", ""))
            ...
            if pysm_icons:
                try:
                    student_item.setIcon(
                        0,
                        pysm_icons.get_qicon("PHOTO_PORTRAIT_EDIT", 20, color="Green"),
                    )
                except Exception:
            ...
            for location in locations:
                ...
                for record in records:
                    ...
                    if pysm_icons:
                        try:
                            photo_item.setIcon(4, pysm_icons.get_qicon("FOLDER", 20))
                        except Exception:
                            logger.debug("Не удалось создать иконку папки", exc_info=True)
                    for relative, origin, physical_paths in files:
                        file_item = QTreeWidgetItem(photo_item, ("", "", "", origin, str(relative)))
                        file_item.setData(0, ITEM_PATHS_ROLE, [str(path) for path in physical_paths])
                        self._set_file_origin_indicator(file_item, 3, origin)
                        file_item.setToolTip(4, "\n".join(str(path) for path in physical_paths))
