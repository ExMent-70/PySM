...
После успешного завершения скрипт записывает маркер выполнения в контекст PySM:
`var_claster_run.<wf_photo_session>.location = "yes"`. Имя фотосессии берётся
из переменной контекста `wf_photo_session`.

## 2. Основные параметры запуска
...
* `Masks/Cutout/` - папка с масками, если анализ выполняется по маскам.

Поддерживаемые расширения входных изображений: `.jpg`, `.jpeg`, `.png`.

При декодировании PNG скрипт удаляет встроенный chunk `iCCP` только из копии
байтов в памяти. Цветовой профиль не используется моделью, а его удаление до
передачи данных в OpenCV предотвращает предупреждение libpng
`iCCP: known incorrect sRGB profile`. Исходные PNG-файлы на диске не изменяются.
...
* Маркер завершения location-анализа записывается по `wf_photo_session`, как и
  остальные этапы Workflow2.
