# run_select_photo.py

"""
Интерактивный скрипт для пакетной обработки фотографий.

Основные функции:
1.  **Парсинг:** Извлекает номера фотографий из произвольного текста.
2.  **Поиск:** Находит соответствующие файлы (RAW, JPG, XMP) в исходной папке на диске.
3.  **Обработка:** Копирует файлы в целевую структуру папок с опциональным переименованием.
4.  **Синхронизация:** Обновляет файл метаданных соответствия портретов и групповых фото.
"""

# ==============================================================================
# 1. БЛОК: Импорты и настройка окружения
# ==============================================================================
import argparse
import json
import logging
import pathlib
import re
import shutil
import sys
from argparse import Namespace
from collections import defaultdict
from typing import Any, Optional

# Настройка путей для импорта локальных пакетов
try:
    current_script_path = pathlib.Path(__file__).resolve()
    project_root = current_script_path.parent.parent
    if str(project_root) not in sys.path:
        sys.path.insert(0, str(project_root))
    from matches_sync import atomic_write_json, sync_matches_data
    from student_identity import (
        StudentRoster,
        build_rename_stem,
        collect_photo_identity,
        format_students_for_table,
        load_student_roster,
        validate_identity_contract,
    )
except ImportError as e:
    print(f"Критическая ошибка импорта: {e}", file=sys.stderr)
    sys.exit(1)

# Настройка базового логгера
logging.basicConfig(level=logging.INFO, format="%(message)s", stream=sys.stdout)
logger = logging.getLogger(__name__)

# Безопасный импорт специфичной библиотеки PySM
try:
    from pysm_lib import pysm_context, theme_api
    from pysm_lib.pysm_context import ConfigResolver
    from pysm_lib.pysm_theme_api import format_ipymarkup_box, set_widget_class
    IS_MANAGED_RUN = True
except ImportError:
    (IS_MANAGED_RUN, ConfigResolver, pysm_context, 
     theme_api, format_ipymarkup_box, set_widget_class) = (False, None, None, None, None, None)

try:
    import ijson
except ImportError:
    print("Ошибка: ijson не найден. Установите зависимость.", file=sys.stderr)
    sys.exit(1)

try:
    from ipymarkup.palette import Palette
except ImportError:
    Palette = None

# Импорт компонентов PySide6 для графического интерфейса
try:
    from PySide6.QtCore import QAbstractItemModel, QModelIndex, Qt, Signal, QThread, QSortFilterProxyModel
    from PySide6.QtGui import QColor
    from PySide6.QtWidgets import (
        QApplication, QCheckBox, QHBoxLayout, QHeaderView, QLabel, QLineEdit,
        QMainWindow, QMessageBox, QPushButton, QSplitter, QStatusBar,
        QStyle, QTabWidget, QTextBrowser, QTextEdit, QTreeView, QVBoxLayout, QWidget,
        QProgressBar
    )
except ImportError:
    print("Ошибка: PySide6 не найден. Установите зависимость.", file=sys.stderr)
    sys.exit(1)


# ==============================================================================
# 2. БЛОК: Получение конфигурации
# ==============================================================================
def get_config() -> Namespace:
    """
    Парсит аргументы командной строки и разрешает конфигурацию через PySM Context.

    Returns:
        Namespace: Объект с конфигурационными параметрами (пути, флаги, настройки).
    """
    parser = argparse.ArgumentParser(description="Пакетная обработка файлов по номерам.")
    
    parser.add_argument(
        "--analysis_dir", type=str, required=True, 
        help="Путь к папке с результатами анализа (содержит JSON)."
    )
    parser.add_argument(
        "--source_dir", type=str, required=True, 
        help="Папка с исходными фотографиями (откуда искать)."
    )
    parser.add_argument(
        "--student_list_file", type=str, required=True,
        help="Файл *.list — единственный источник фамилии и имени."
    )
    parser.add_argument(
        "--dest_dir", type=str, required=False, 
        help="Корневая папка для результатов (куда копировать)."
    )
    parser.add_argument(
        "--min_digits", type=int, default=4, 
        help="Минимальная длина извлекаемого номера."
    )
    parser.add_argument(
        "--max_digits", type=int, default=4, 
        help="Максимальная длина извлекаемого номера."
    )
    parser.add_argument(
        "--exclude_dirs", nargs="+", default=["CaptureOne"], 
        help="Имена папок, которые будут проигнорированы при поиске."
    )
    parser.add_argument(
        "--rename-files", action="store_true", 
        help="Активировать переименование файлов при копировании."
    )
    parser.add_argument(
        "--copy-files", action="store_true", 
        help="Активировать физическое копирование файлов на диск."
    )

    if IS_MANAGED_RUN and ConfigResolver:
        return ConfigResolver(parser).resolve_all()
    return parser.parse_args()


# ==============================================================================
# 3. БЛОК: Вспомогательные функции (I/O данных)
# ==============================================================================
def load_json(json_path: pathlib.Path, required: bool = True) -> dict:
    """
    Безопасно читает JSON файл с обработкой ошибок формата и доступа.

    Args:
        json_path (pathlib.Path): Путь к файлу JSON.
        required (bool): Если True, выбрасывает исключение при отсутствии файла. 
                         Если False, возвращает пустой словарь.

    Returns:
        dict: Распарсенные данные из JSON.
        
    Raises:
        FileNotFoundError: Если файл не найден и required=True.
        ValueError: Если файл содержит невалидный JSON.
        RuntimeError: При любых других системных ошибках чтения.
    """
    if not json_path or not json_path.exists():
        if required:
            raise FileNotFoundError(f"Файл не найден: {json_path}")
        return dict()
        
    try:
        with open(json_path, "r", encoding="utf-8") as f:
            return json.load(f)
    except json.JSONDecodeError as e:
        raise ValueError(f"Ошибка формата JSON в файле {json_path.name}: {e}")
    except Exception as e:
        raise RuntimeError(f"Неизвестная ошибка чтения {json_path.name}: {e}")


# ==============================================================================
# 4. БЛОК: Бизнес-логика (Сервисы)
# ==============================================================================
class DataSyncService:
    """
    Сервис для синхронизации результатов поиска с JSON-хранилищем проекта.
    Отвечает за обновление файла matches_portrait_to_group.json.
    """
    def __init__(self, matches_json_path: pathlib.Path):
        """
        Инициализирует сервис синхронизации.
        
        Args:
            matches_json_path (pathlib.Path): Путь к файлу matches_portrait_to_group.json.
        """
        self.matches_json_path = matches_json_path
        # Файл может отсутствовать при первом запуске, поэтому required=False
        self.matches_data = load_json(matches_json_path, required=False) if matches_json_path else dict()

    def sync(self, search_results: list[dict], selected_numbers: set[str], config: Namespace) -> bool:
        """
        Основной метод синхронизации. Обновляет списки групповых фото для персон.

        Args:
            search_results (list[dict]): Список словарей с результатами поиска файлов.
            selected_numbers (set[str]): Множество номеров, которые пользователь выбрал для обработки.
            config (Namespace): Конфигурация для извлечения настроек длины номеров.
        """
        if not self.matches_data or not self.matches_json_path:
            logger.warning("Файл matches.json пуст или не загружен. Синхронизация пропущена.")
            return False

        self.matches_data = sync_matches_data(
            self.matches_data,
            search_results,
            selected_numbers,
            config.min_digits,
            config.max_digits,
        )
        atomic_write_json(self.matches_json_path, self.matches_data)
        logger.info(f"Файл успешно сохранен: {self.matches_json_path.name}")
        return True


# ==============================================================================
# 5. БЛОК: Модель данных GUI (Qt Model/View)
# ==============================================================================
class TreeItem:
    """Узел древовидной структуры данных для модели отображения в QTreeView."""
    
    def __init__(
        self,
        data: list,
        parent: "TreeItem" = None,
        tooltips: dict[int, str] | None = None,
    ):
        """
        Args:
            data (list): Данные столбцов текущего узла.
            parent (TreeItem, optional): Родительский узел дерева.
        """
        self._data = data
        self._parent = parent
        self._children: list["TreeItem"] = list()
        self._tooltips = tooltips or {}

    def child(self, row: int) -> Optional["TreeItem"]:
        """Возвращает дочерний элемент по индексу строки."""
        return self._children[row] if 0 <= row < len(self._children) else None

    def childCount(self) -> int:
        """Возвращает количество дочерних элементов."""
        return len(self._children)

    def columnCount(self) -> int:
        """Возвращает количество столбцов (атрибутов) узла."""
        return len(self._data)

    def data(self, column: int) -> Any:
        """Возвращает данные конкретного столбца."""
        return self._data[column] if 0 <= column < len(self._data) else None

    def tooltip(self, column: int) -> str:
        """Возвращает подсказку столбца, если она задана для узла."""

        return self._tooltips.get(column, "")

    def parent(self) -> Optional["TreeItem"]:
        """Возвращает родительский элемент."""
        return self._parent

    def row(self) -> int:
        """Определяет свой индекс в списке детей родителя."""
        return self._parent._children.index(self) if self._parent else 0

    def appendChild(self, item: "TreeItem") -> None:
        """Добавляет новый дочерний узел."""
        self._children.append(item)


class ResultTreeModel(QAbstractItemModel):
    """
    Модель данных (AbstractItemModel) для отображения иерархического 
    списка найденных файлов в таблице (QTreeView).
    """
    
    def __init__(self, data=None, icons=None, colors=None, parent=None):
        super().__init__(parent)
        self.headers =[
            "№", "Номер", "Ученики", "Локация", "Статус", "Новое имя (основа)", "Файлы"
        ]
        self._root_item = TreeItem(self.headers)
        self.icons = icons or dict()
        self.colors = colors or dict()
        self.setup_model_data(data or list())

    def setup_model_data(self, model_data: list[dict]) -> None:
        """
        Строит иерархию узлов (TreeItem) на основе списка словарей результатов.

        Args:
            model_data (list[dict]): Данные результатов поиска.
        """
        self.beginResetModel()
        self._root_item = TreeItem(self.headers)
        
        for idx, item_data in enumerate(model_data, 1):
            files = item_data.get("files", list())
            base_path = item_data.get("base_path")
            files_count_str = f"({len(files)} файлов)" if files else ""
            
            # ВАЖНО: Приводим строковый номер к int, если это число.
            # Это позволяет QSortFilterProxyModel применять правильную математическую 
            # сортировку (2 будет стоять перед 10), а не алфавитную.
            num_str = item_data.get("number", "")
            num_val = int(num_str) if num_str.isdigit() else num_str
            students_text, students_tooltip = format_students_for_table(
                item_data.get("student_names", [])
            )
            
            row_data =[
                idx, 
                num_val,
                students_text,
                item_data.get("location_name", ""),
                item_data.get("status", "Ожидает"),
                item_data.get("new_stem", ""),
                files_count_str,
            ]
            parent_item = TreeItem(
                row_data,
                self._root_item,
                tooltips={2: students_tooltip} if students_tooltip else None,
            )

            # Если есть вложенные файлы, добавляем их как дочерние элементы строки
            if files and base_path:
                for fpath in files:
                    try:
                        rel_path = fpath.relative_to(base_path)
                    except ValueError:
                        rel_path = fpath.name
                    
                    child_row_data =["", "", "", "", "", "", str(rel_path)]
                    parent_item.appendChild(TreeItem(child_row_data, parent_item))

            self._root_item.appendChild(parent_item)
            
        self.endResetModel()

    def data(self, index: QModelIndex, role: int) -> Any:
        """Возвращает данные для отображения в ячейках таблицы."""
        if not index.isValid():
            return None
            
        item = index.internalPointer()
        col = index.column()

        # Роль отображения текста
        if role == Qt.DisplayRole:
            return item.data(col)

        if role == Qt.ToolTipRole:
            return item.tooltip(col) or None

        # Роль отображения иконок в столбце "Файлы"
        if role == Qt.DecorationRole and col == 6:
            if item.parent() == self._root_item and item.childCount() > 0:
                return self.icons.get("folder")
            elif item.parent() != self._root_item:
                return self.icons.get("file")

        # Роль цвета фона и текста для столбца "Статус"
        if col == 4:
            status = item.data(4)
            is_found = status == "Найден"
            is_not_found = status == "Не найден"
            
            if role == Qt.BackgroundRole:
                if is_found: return self.colors.get("found_bg")
                if is_not_found: return self.colors.get("not_found_bg")
            if role == Qt.ForegroundRole:
                if is_found: return self.colors.get("found_fg")
                if is_not_found: return self.colors.get("not_found_fg")

        return None

    def index(self, row: int, col: int, parent: QModelIndex = QModelIndex()) -> QModelIndex:
        """Создает индекс для ячейки модели."""
        if not self.hasIndex(row, col, parent):
            return QModelIndex()
            
        parent_item = parent.internalPointer() if parent.isValid() else self._root_item
        child_item = parent_item.child(row)
        return self.createIndex(row, col, child_item) if child_item else QModelIndex()

    def parent(self, index: QModelIndex) -> QModelIndex:
        """Возвращает индекс родителя для текущей ячейки."""
        if not index.isValid():
            return QModelIndex()
            
        child_item = index.internalPointer()
        parent_item = child_item.parent()
        
        if parent_item == self._root_item:
            return QModelIndex()
        return self.createIndex(parent_item.row(), 0, parent_item)

    def rowCount(self, parent: QModelIndex = QModelIndex()) -> int:
        """Возвращает количество строк (потомков)."""
        if parent.column() > 0:
            return 0
        parent_item = parent.internalPointer() if parent.isValid() else self._root_item
        return parent_item.childCount()

    def columnCount(self, parent: QModelIndex = QModelIndex()) -> int:
        """Возвращает количество столбцов."""
        return len(self.headers)

    def headerData(self, section: int, orientation: Qt.Orientation, role: int) -> Any:
        """Возвращает заголовки столбцов."""
        if orientation == Qt.Horizontal and role == Qt.DisplayRole:
            return self._root_item.data(section)
        return None


# ==============================================================================
# 6. БЛОК: Изолированные Воркеры (Фоновые потоки QThread)
# ==============================================================================
class ParseWorker(QThread):
    """
    Фоновый поток для парсинга текста. 
    Извлекает номера определенной длины с помощью регулярных выражений.
    """
    finished_signal = Signal(list, str)
    error_signal = Signal(str)

    def __init__(self, text: str, min_digits: int, max_digits: int, markup_color: str):
        super().__init__()
        self.text = text
        self.min_digits = min_digits
        self.max_digits = max_digits
        self.markup_color = markup_color

    def run(self) -> None:
        """Выполняет поиск номеров и генерацию HTML с подсветкой ipymarkup."""
        try:
            min_d, max_d = sorted([self.min_digits, self.max_digits])
            # Использование негативного просмотра (?<!\d) и (?!\d) гарантирует,
            # что мы не вырвем 4 цифры из середины 5-значного или 6-значного числа.
            regex = r"(?<!\d)(\d{%d,%d})(?!\d)" % (min_d, max_d)
            
            matches = list(re.finditer(regex, self.text))
            numbers = sorted(list({m.group(1) for m in matches}))
            
            html = ""
            if format_ipymarkup_box and Palette and self.markup_color:
                palette = Palette([self.markup_color])
                spans = [(m.start(), m.end(), "") for m in matches]
                html = format_ipymarkup_box(self.text, spans, palette=palette)
                
            self.finished_signal.emit(numbers, html)
        except Exception as e:
            self.error_signal.emit(f"Ошибка парсинга: {e}")


class SearchWorker(QThread):
    """
    Фоновый поток для ленивого поиска файлов на диске и сопоставления их с метаданными.
    """
    finished_signal = Signal(list)
    error_signal = Signal(str)

    def __init__(
        self, 
        source_dir: pathlib.Path, 
        metadata: dict, 
        student_roster: StudentRoster,
        numbers: list[str], 
        exclude_dirs: list[str], 
        min_digits: int, 
        max_digits: int
    ):
        super().__init__()
        self.source_dir = source_dir
        self.metadata = metadata
        self.student_roster = student_roster
        self.numbers_to_find = set(numbers)
        self.exclude_dirs = set(exclude_dirs)
        self.min_digits = min_digits
        self.max_digits = max_digits

    def run(self) -> None:
        """Сканирует директорию на лету (rglob) для оптимизации использования ОЗУ."""
        try:
            min_d, max_d = sorted([self.min_digits, self.max_digits])
            regex = re.compile(r"(?<!\d)(\d{%d,%d})(?!\d)" % (min_d, max_d))
            
            # Подготовка словаря метаданных только для тех файлов, которые есть в JSON
            number_to_metadata = dict()
            for filename, data in self.metadata.items():
                match = regex.search(filename)
                if match and match.group(1) not in number_to_metadata:
                    number_to_metadata[match.group(1)] = (filename, data)

            files_map = defaultdict(list)
            
            # Ленивый проход по файловой системе (экономия RAM при больших объемах)
            for f in self.source_dir.rglob("*"):
                if not f.is_file():
                    continue
                # Пропускаем папки, указанные в исключениях
                if self.exclude_dirs.intersection(set(f.parts)):
                    continue
                    
                match = regex.search(f.stem)
                if match:
                    num = match.group(1)
                    if num in self.numbers_to_find:
                        files_map[num].append(f)
            
            # Формирование итогового отчета по каждому запрошенному номеру
            results = list()
            for num in self.numbers_to_find:
                metadata_filename, item_data = number_to_metadata.get(
                    num, ("", dict())
                )
                found_files = files_map.get(num, list())
                status = "Найден" if found_files else "Не найден"

                faces = item_data.get("faces", list())
                identity = collect_photo_identity(faces, self.student_roster)
                    
                location_name = item_data.get("location_name", "Не определена")
                
                new_stem = ""
                original_filename = ""
                
                if found_files:
                    new_stem = build_rename_stem(
                        identity, num, found_files[0].stem
                    )
                    # Для JSON используем каноническое имя из info_faces, а не
                    # случайное расширение первого найденного RAW/JPG-файла.
                    original_filename = metadata_filename or found_files[0].name
                else:
                    original_filename = metadata_filename or f"IMG_{num}.jpg"

                results.append({
                    "number": num, 
                    "status": status, 
                    "files": sorted(list(set(found_files))),
                    "student_ids": list(identity.student_ids),
                    "student_names": list(identity.student_names),
                    "display_students": identity.display_students,
                    "rename_person_name": identity.rename_person_name,
                    "location_name": location_name,
                    "base_path": self.source_dir, 
                    "new_stem": new_stem,
                    "original_filename": original_filename
                })
                
            self.finished_signal.emit(results)
        except Exception as e:
            self.error_signal.emit(f"Ошибка поиска: {e}")


class ProcessWorker(QThread):
    """
    Фоновый поток для физического копирования и переименования файлов на диске.
    Обеспечивает передачу прогресса выполнения в основной поток GUI.
    """
    finished_signal = Signal(str)
    progress_signal = Signal(str)
    progress_percent_signal = Signal(int)
    error_signal = Signal(str)
    failed_signal = Signal(str)

    def __init__(
        self, 
        search_results: list[dict], 
        source_dir: pathlib.Path, 
        dest_dir: pathlib.Path, 
        do_rename: bool, 
        do_copy: bool
    ):
        super().__init__()
        self.search_results = search_results
        self.source_dir = source_dir
        self.dest_dir = dest_dir
        self.do_rename = do_rename
        self.do_copy = do_copy

    def run(self) -> None:
        """Создает структуру папок и выполняет копирование (shutil.copy2)."""
        try:
            if not self.do_copy:
                self.finished_signal.emit("Нет задач для выполнения (копирование отключено).")
                return

            if not self.dest_dir:
                self.failed_signal.emit("Не указана папка назначения (--dest_dir).")
                return

            items_to_process =[
                item for item in self.search_results 
                if item.get("status") == "Найден"
            ]
            total = len(items_to_process)
            copied_count = 0
            copy_errors: list[str] = []
            
            for i, item in enumerate(items_to_process, 1):
                files = item.get("files", list())
                location = item.get("location_name", "unknown")
                # Создаем подпапку локации внутри целевой директории
                target_location_root = self.dest_dir / str(location)
                new_stem = item.get("new_stem") if self.do_rename else None

                for src_file in files:
                    try:
                        rel_path = src_file.relative_to(self.source_dir)
                    except ValueError:
                        rel_path = pathlib.Path(src_file.name)
                    
                    # Восстанавливаем оригинальную структуру вложенности исходника
                    dest_folder = target_location_root / rel_path.parent
                    dest_folder.mkdir(parents=True, exist_ok=True)
                    
                    dest_name = f"{new_stem}{src_file.suffix}" if new_stem else src_file.name
                    dest_file = dest_folder / dest_name
                    
                    if not dest_file.exists():
                        try:
                            shutil.copy2(src_file, dest_file)
                            copied_count += 1
                        except Exception as e:
                            copy_errors.append(
                                f"{src_file.name}: {e}"
                            )
                            
                # Вычисляем и передаем процент завершения
                percent = int((i / total) * 100) if total > 0 else 100
                self.progress_percent_signal.emit(percent)
                self.progress_signal.emit(f"Скопировано: группа {i} из {total}")
            
            if copy_errors:
                details = "\n".join(copy_errors[:10])
                self.failed_signal.emit(
                    f"Не удалось скопировать файлов: {len(copy_errors)}.\n{details}"
                )
                return
            self.finished_signal.emit(
                f"Обработка завершена. Скопировано файлов: {copied_count}; "
                f"обработано групп: {total}."
            )
        except Exception as e:
            self.failed_signal.emit(f"Ошибка копирования: {e}")


# ==============================================================================
# 7. БЛОК: Главное окно графического интерфейса
# ==============================================================================
class SorterWindow(QMainWindow):
    """Главное окно приложения на базе PySide6. Управляет связкой UI и бизнес-логики."""
    
    def __init__(self, config: Namespace):
        super().__init__()
        self.config = config
        self.final_status = 1
        self.search_results = list()
        
        # Резервируем ссылки на потоки
        self._parse_worker = None
        self._search_worker = None
        self._process_worker = None

        # Инициализация слоя бизнес-логики и загрузка данных
        try:
            analysis_dir = pathlib.Path(config.analysis_dir)
            self.student_roster = load_student_roster(
                pathlib.Path(config.student_list_file)
            )
            faces_json_path = analysis_dir / "info_faces.json"
            self.metadata = load_json(faces_json_path, required=True)
            
            matches_path = analysis_dir / "matches_portrait_to_group.json"
            self.sync_service = DataSyncService(matches_path)
            validate_identity_contract(
                self.metadata,
                self.sync_service.matches_data,
                self.student_roster,
            )
            
        except Exception as e:
            QMessageBox.critical(self, "Критическая ошибка", f"Не удалось загрузить данные:\n{e}")
            self.setEnabled(False)
            self.metadata = dict()
            self.student_roster = None
            self.sync_service = None

        self._load_theme_colors()
        self._init_ui()

    def _load_theme_colors(self) -> None:
        """Загружает цвета из активной темы PySM, либо устанавливает дефолтные."""
        if IS_MANAGED_RUN and theme_api:
            default_markup = {
                "background-color": "#e3f2fd", 
                "border-color": "#bbdefb", 
                "color": "#000000"
            }
            self.ipymarkup_color = theme_api.get_ipymarkup_color("markup_number", defaults=default_markup)
            
            self.colors = {
                "found_bg": theme_api.get_qcolor(
                    "table_status_found", "background-color", "#d4edda"
                ),
                "found_fg": theme_api.get_qcolor(
                    "table_status_found", "color", "#155724"
                ),
                "not_found_bg": theme_api.get_qcolor(
                    "table_status_not_found", "background-color", "#f8d7da"
                ),
                "not_found_fg": theme_api.get_qcolor(
                    "table_status_not_found", "color", "#721c24"
                ),
            }
        else:
            self.ipymarkup_color = None
            self.colors = {
                "found_bg": QColor("#d4edda"), 
                "found_fg": QColor("#155724"), 
                "not_found_bg": QColor("#f8d7da"), 
                "not_found_fg": QColor("#721c24")
            }

    def _init_ui(self) -> None:
        """Настраивает графический интерфейс: виджеты, таблицы, кнопки и события."""
        title = "Сортировка по локациям"
        source_dir = pathlib.Path(self.config.source_dir)
        if source_dir: 
            title += f" - [{source_dir.name}]"
            
        self.setWindowTitle(title)
        self.resize(1200, 750)

        # Инициализация иконок ДО инициализации модели таблицы
        self.icons = {
            "folder": self.style().standardIcon(QStyle.SP_DirIcon), 
            "file": self.style().standardIcon(QStyle.SP_FileIcon)
        }

        main_widget = QWidget()
        self.setCentralWidget(main_widget)
        layout = QVBoxLayout(main_widget)
        splitter = QSplitter(Qt.Horizontal)
        layout.addWidget(splitter)

        # Левая панель: Ввод и просмотр HTML
        self.left_tabs = QTabWidget()
        input_tab = QWidget()
        l_in = QVBoxLayout(input_tab)
        l_in.addWidget(QLabel("Вставьте текст с номерами:"))
        self.text_input = QTextEdit()
        l_in.addWidget(self.text_input)
        self.left_tabs.addTab(input_tab, "Ввод")
        
        self.markup_browser = QTextBrowser()
        self.markup_browser.setOpenExternalLinks(True)
        self.left_tabs.addTab(self.markup_browser, "Разбор")

        # Правая панель: Таблица результатов и строка выбранных номеров
        right_panel = QWidget()
        l_right = QVBoxLayout(right_panel)
        l_right.addWidget(QLabel("Результаты анализа:"))
        
        self.tree_view = QTreeView()
        self.tree_view.setAlternatingRowColors(True)
        self.tree_view.setSortingEnabled(True)
        
        # Использование Proxy-модели для быстрой и правильной сортировки данных
        self.proxy_model = QSortFilterProxyModel()
        self.tree_view.setModel(self.proxy_model)
        self._set_empty_model() 
        
        l_right.addWidget(self.tree_view)
        
        l_right.addWidget(QLabel("Найденные номера для копирования:"))
        self.numbers_line_edit = QLineEdit()
        self.numbers_line_edit.setReadOnly(True)
        l_right.addWidget(self.numbers_line_edit)

        # Сплиттер для регулировки ширины панелей
        splitter.addWidget(self.left_tabs)
        splitter.addWidget(right_panel)
        splitter.setSizes([400, 800])

        # Нижняя панель: Кнопки управления
        bottom_layout = QHBoxLayout()
        self.btn_parse = QPushButton("1. Извлечь номера")
        self.btn_search = QPushButton("2. Найти файлы")
        
        self.btn_copy = QPushButton("3. Копировать файлы")
        set_widget_class(self.btn_copy, "primary")
        
        self.chk_sync_matches = QCheckBox("Синхронизировать matches.json")
        self.chk_sync_matches.setChecked(False)

        self.btn_close = QPushButton("Закрыть")
        set_widget_class(self.btn_close, "danger")
        self.btn_close.clicked.connect(self.close)

        bottom_layout.addWidget(self.btn_parse)
        bottom_layout.addWidget(self.btn_search)
        bottom_layout.addStretch()
        bottom_layout.addWidget(self.chk_sync_matches)
        bottom_layout.addWidget(self.btn_copy)
        bottom_layout.addWidget(self.btn_close)
        layout.addLayout(bottom_layout)

        self.btn_search.setEnabled(False)
        self.btn_copy.setEnabled(False)
        
        # Статус-бар и индикатор прогресса
        self.setStatusBar(QStatusBar())
        self.progress_bar = QProgressBar()
        self.progress_bar.setMinimumWidth(200)
        self.progress_bar.setVisible(False)
        self.statusBar().addPermanentWidget(self.progress_bar)

        # Привязка событий к кнопкам
        self.btn_parse.clicked.connect(self.start_parse)
        self.btn_search.clicked.connect(self.start_search)
        self.btn_copy.clicked.connect(self.start_processing)

    def _set_empty_model(self) -> None:
        """Сбрасывает данные таблицы до пустого состояния."""
        empty_model = ResultTreeModel(list(), self.icons, getattr(self, "colors", dict()))
        self.proxy_model.setSourceModel(empty_model)

    def start_parse(self) -> None:
        """Запускает процесс извлечения номеров из введенного текста."""
        text = self.text_input.toPlainText()
        if not text.strip():
            return
        
        self.numbers_line_edit.clear()
        self.statusBar().showMessage("Парсинг текста...")
        self.progress_bar.setRange(0, 0)  # Включение режима "пульсации"
        self.progress_bar.setVisible(True)
        
        self._parse_worker = ParseWorker(
            text, 
            self.config.min_digits, 
            self.config.max_digits, 
            self.ipymarkup_color
        )
        self._parse_worker.finished_signal.connect(self.on_parse_finished)
        self._parse_worker.error_signal.connect(
            lambda msg: self.statusBar().showMessage(msg)
        )
        self._parse_worker.start()

    def on_parse_finished(self, numbers: list[str], html: str) -> None:
        """Обработчик успешного завершения парсинга."""
        self.progress_bar.setVisible(False)
        self.statusBar().showMessage(f"Найдено номеров: {len(numbers)}")
        self.markup_browser.setHtml(html)
        self.numbers_line_edit.setText(" ".join(numbers))
        
        self.btn_search.setEnabled(bool(numbers))
        self._set_empty_model()
        
        if numbers:
            # Автоматически переключаем на вкладку "Разбор"
            self.left_tabs.setCurrentIndex(1)

    def start_search(self) -> None:
        """Запускает поиск файлов на диске по извлеченным номерам."""
        if not self.isEnabled():
            return
            
        numbers = self.numbers_line_edit.text().split()
        if not numbers:
            return
        
        self.statusBar().showMessage("Поиск и анализ файлов на диске...")
        self.progress_bar.setRange(0, 0)
        self.progress_bar.setVisible(True)
        
        self._search_worker = SearchWorker(
            source_dir=pathlib.Path(self.config.source_dir),
            metadata=self.metadata,
            student_roster=self.student_roster,
            numbers=numbers,
            exclude_dirs=getattr(self.config, "exclude_dirs", list()),
            min_digits=self.config.min_digits,
            max_digits=self.config.max_digits
        )
        self._search_worker.finished_signal.connect(self.on_search_finished)
        self._search_worker.error_signal.connect(
            lambda msg: self.statusBar().showMessage(msg)
        )
        self._search_worker.start()

    def on_search_finished(self, results: list[dict]) -> None:
        """Обработчик успешного завершения поиска файлов."""
        self.progress_bar.setVisible(False)
        self.search_results = results
        self.statusBar().showMessage(f"Анализ завершен. Найдено групп: {len(results)}")
        
        model = ResultTreeModel(results, self.icons, getattr(self, "colors", dict()))
        self.proxy_model.setSourceModel(model)
        
        self.tree_view.collapseAll()
        self.tree_view.sortByColumn(0, Qt.AscendingOrder)
        
        for i in range(model.columnCount()):
            self.tree_view.resizeColumnToContents(i)
        self.tree_view.header().setSectionResizeMode(5, QHeaderView.Stretch)
        
        found_items_count = sum(1 for r in results if r.get("status") == "Найден")
        self.btn_copy.setEnabled(found_items_count > 0)
        self.btn_copy.setText(f"3. Копировать ({found_items_count} групп)")

    def start_processing(self) -> None:
        """Запускает основные операции копирования и переименования."""
        if not self.search_results:
            QMessageBox.warning(self, "Внимание", "Нет файлов для обработки.")
            return

        do_rename_files = getattr(self.config, "rename_files", False)
        do_copy_files = getattr(self.config, "copy_files", False)

        if not do_copy_files:
            QMessageBox.information(
                self, 
                "Информация", 
                "Операция копирования не была включена в параметрах запуска."
            )
            return
            
        if not self.config.dest_dir:
            QMessageBox.critical(
                self, 
                "Ошибка", 
                "Операция копирования включена, но не указана папка назначения."
            )
            return

        mode_str = "с переименованием" if do_rename_files else "без переименования"
        msg = f"Будет выполнено копирование {mode_str}.\n\nПродолжить?"
        
        if QMessageBox.question(self, "Подтверждение", msg) == QMessageBox.No:
            return

        self.statusBar().showMessage("Подготовка к копированию...")
        self.btn_copy.setEnabled(False)
        
        self.progress_bar.setRange(0, 100)
        self.progress_bar.setValue(0)
        self.progress_bar.setVisible(True)
        
        self._process_worker = ProcessWorker(
            search_results=self.search_results,
            source_dir=pathlib.Path(self.config.source_dir),
            dest_dir=pathlib.Path(self.config.dest_dir),
            do_rename=do_rename_files,
            do_copy=do_copy_files
        )
        self._process_worker.finished_signal.connect(self.on_processing_finished)
        self._process_worker.failed_signal.connect(self.on_processing_failed)
        self._process_worker.progress_signal.connect(
            lambda msg: self.statusBar().showMessage(msg)
        )
        self._process_worker.progress_percent_signal.connect(self.progress_bar.setValue)
        self._process_worker.error_signal.connect(
            lambda msg: self.statusBar().showMessage(msg)
        )
        self._process_worker.start()

    def on_processing_failed(self, message: str) -> None:
        """Возвращает GUI в рабочее состояние после ошибки копирования."""

        self.progress_bar.setVisible(False)
        self.statusBar().showMessage(message)
        self.btn_copy.setEnabled(True)
        self.final_status = 1
        QMessageBox.critical(self, "Ошибка копирования", message)

    def on_processing_finished(self, message: str) -> None:
        """Обрабатывает завершение всех операций и очищает интерфейс."""
        self.progress_bar.setVisible(False)
        self.statusBar().showMessage(message)

        sync_completed = False
        # Синхронизация данных JSON, если включена опция
        if self.chk_sync_matches.isChecked() and self.sync_service:
            selected_numbers = set(self.numbers_line_edit.text().split())
            try:
                sync_completed = self.sync_service.sync(
                    self.search_results, selected_numbers, self.config
                )
            except Exception as exc:
                self.final_status = 1
                QMessageBox.critical(
                    self,
                    "Ошибка синхронизации",
                    f"Файлы скопированы, но matches не обновлён:\n{exc}",
                )
                self.btn_copy.setEnabled(True)
                return

        self.final_status = 0

        # Очистка интерфейса для следующей задачи
        self.markup_browser.clear()
        self.numbers_line_edit.clear()
        self._set_empty_model()
        self.search_results = list()
        
        self.btn_search.setEnabled(False)
        self.btn_copy.setEnabled(False)
        self.left_tabs.setCurrentIndex(0)

        sync_text = " Matches синхронизирован." if sync_completed else ""
        QMessageBox.information(
            self, "Успех", f"Все операции успешно завершены.{sync_text}"
        )
        
        # Логирование ссылки для платформы PySM
        if IS_MANAGED_RUN and pysm_context and getattr(self.config, "copy_files", False):
            dest_dir = pathlib.Path(self.config.dest_dir)
            if dest_dir.exists():
                pysm_context.log_link(
                    url_or_path=str(dest_dir),
                    text=f"<br>Открыть папку с результатами ('{dest_dir.name}')",
                )


# ==============================================================================
# 8. БЛОК: Точка входа в приложение
# ==============================================================================
def main() -> None:
    """Инициализация и запуск графического приложения Qt."""
    config = get_config()
    app = QApplication.instance() or QApplication(sys.argv)
    
    if IS_MANAGED_RUN and theme_api:
        theme_api.apply_theme_to_app(app)

    window = SorterWindow(config)
    window.show()

    exit_code = app.exec()
    sys.exit(window.final_status if exit_code == 0 else exit_code)

if __name__ == "__main__":
    main()
