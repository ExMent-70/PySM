...
class ConfigManager:
    ...
    def _load_and_validate(self) -> Dict[str, Any]:
        ...
        if not self.config_path.is_file():
            logger.error(f"{icon_error} Файл конфигурации не найден: {self.config_path.resolve()}")
            raise FileNotFoundError(f"{icon_error} Config file not found: {self.config_path.resolve()}")
        try:
            config_data = toml.load(self.config_path)
            validated_config = AppConfig(**config_data).model_dump(mode="python")
            logger.debug(f"Дополнительные настройки скрипта загружены из файла {self.config_path.name}.")
            return validated_config
        except ValidationError as e:
            logger.error(f"{icon_error} Ошибка валидации конфигурации в {self.config_path.name}:\n{e}")
            raise
        except Exception as e:
            logger.error(f"{icon_error} Не удалось загрузить или прочитать файл конфигурации: {e}")
            raise
