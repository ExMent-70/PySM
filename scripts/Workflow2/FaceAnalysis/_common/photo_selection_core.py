...
def _validated_location(
    ...
    """Return one safe Windows directory component or record a blocker."""

    location = str(value or "unknown").strip() or "unknown"
    candidate = PureWindowsPath(location)
    invalid = (
        candidate.is_absolute()
        or len(candidate.parts) != 1
        or candidate.name != location
        or candidate.is_reserved()
        or location in {".", ".."}
        or location.rstrip(" .") != location
        or any(char in '<>:"/\\|?*' or ord(char) < 32 for char in location)
    )
    if invalid:
        issues.append(Issue(
            "error",
            "invalid_location",
            f"Некорректная локация {location!r}: {filename}",
        ))
        return "unknown"
    return location
...
def index_records_by_student_location(
    result: BuildResult,
) -> dict[tuple[str, str], list[PhotoRecord]]:
    """Index assigned records in one pass for GUI and export consumers."""

    index: dict[tuple[str, str], list[PhotoRecord]] = {}
    for record in result.records.values():
        for student_id in record.assigned_student_ids:
            index.setdefault((student_id, record.location), []).append(record)
