# Руководство по "Диалогу выбора из списка" (v2.2026.07.07)
...
-   **Имя переменной результата** (`dlg_choice_var`):
    ...
    -   Поддерживается точечная нотация для вложенных JSON-значений, например `project.choice`.
...
Для сохранения во вложенное поле JSON-переменной можно указать имя вида
`project.watermark_type`.
