# Руководство по использованию скрипта "Диалоговое окно" (v2.2026.07.07)
...
-   **Имя переменной результата** (`dlg_msg_var`):
    ...
    -   Поддерживается точечная нотация для вложенных JSON-значений, например `project.confirmation`.
...
Для сохранения ответа во вложенное поле JSON-переменной можно указать имя вида
`project.confirm_processing`.
