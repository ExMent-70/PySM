# Руководство по использованию скрипта "Выбор файла или папки" (v2.2026.07.07)
...
-   **Имя переменной пути** (`dlg_open_var`):
    ...
    -   Поддерживается точечная нотация для вложенных JSON-значений, например `project.source_photo_dir`.
...
Для сохранения пути во вложенное поле JSON-переменной можно указать имя вида
`project.source_photo_dir`. Для обычной переменной сохраняется тип `file_path`
или `dir_path`; для вложенного поля сохраняется строковое значение пути.
